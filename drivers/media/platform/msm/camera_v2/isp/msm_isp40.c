...
{
	if (irq_status1 & (1 << 8)) {
		pr_err("%s: halt_complete\n", __func__); 
		trace_printk("isp_dbg: %s halt_complete, irq_status1: 0x%x\n", __func__, irq_status1);
	...
	struct msm_isp_timestamp *ts)
{
	...
	if (irq_status0 & BIT(2)) {
		...
		trace_printk("%s: EPOCH0 IRQ vfe %d frame_id %d irq_status0 0x%x irq_status1 0x%x \n", __func__,
		vfe_dev->pdev->id,
		vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id,
	...
	uint32_t blocking)
{
	...
	if (blocking) { 
		reinit_completion(&vfe_dev->halt_complete); 

#ifdef CONFIG_ARM64
		pr_err("%s:VFE%d halt, comp lock owner %d / next %d\n", __func__, vfe_dev->pdev->id, vfe_dev->halt_complete.wait.lock.rlock.raw_lock.owner, vfe_dev->halt_complete.wait.lock.rlock.raw_lock.next);
#endif
		/* Halt AXI Bus Bridge */
		msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x2C0);
		trace_printk("isp_dbg: %s wait_for_completion_interruptible_timeout vfe %d \n", __func__, vfe_dev->pdev->id);
		rc = wait_for_completion_timeout(

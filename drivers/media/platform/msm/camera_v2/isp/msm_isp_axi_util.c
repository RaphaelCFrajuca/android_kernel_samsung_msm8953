		...
		if (vfe_dev_cur->common_data->dual_vfe_res->
			...
			pr_err("Missing epoch: vfe %d, epoch mask 0x%x\n",
				...
				vfe_dev_cur->common_data->dual_vfe_res->
					epoch_sync_mask);
			trace_printk("missing epoch: vfe %d, frame_id %d epoch mask 0x%x\n",
         	vfe_dev_cur->pdev->id,
         	vfe_dev_cur->axi_data.src_info[VFE_PIX_0].frame_id,
         	vfe_dev_cur->common_data->dual_vfe_res->epoch_sync_mask);			
...
fatal:
	spin_unlock_irqrestore(
		&vfe_dev_cur->common_data->common_dev_data_lock, flags);
	if (vfe_dev_cur->axi_data.stream_update[frame_src] != 0) { 
			...
			enum msm_isp_camif_update_state camif_update)
{
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		trace_printk("%s, Stream 0x%x src %d src_state %d on vfe %d\n",
			__func__, stream_info->stream_id,
			HANDLE_TO_IDX(stream_cfg_cmd->stream_handle[i]),
			...
			int halt)
{
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		trace_printk("%s: stream 0x%x, vfe %d camif %d halt %d wait %d\n",
			__func__,
			stream_info->stream_id,
			vfe_dev->pdev->id,
			camif_update,

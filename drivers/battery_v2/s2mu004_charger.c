...
static int s2mu004_get_cc301k_mode(char *val)
{
	cc301k_mode = strncmp(val, "1", 1) ? 0 : 1;
	pr_info("%s, cc301k_mode:%d\n", __func__, cc301k_mode);
	return 1;
}
__setup("cc301k_mode=", s2mu004_get_cc301k_mode);
#endif
...
{
	struct s2mu004_charger_data *charger = container_of(work, struct s2mu004_charger_data, otg_vbus_work.work);
